...
function SortableItem({ id, onClick, children }) {
  ...
  const handleClick = (e) => {
    // Evita ejecutar onClick si el clic proviene de elementos interactivos fuera del área de arrastre
...
export default function ParticipantsSection({
  ...
  removeParticipant,
  formatTime,
  onReorder // nueva prop para actualizar el orden
}) {
                ...
                <Col key={p.id} xs={12} sm={6} md={3} lg={3}>
                  <SortableItem id={p.id} onClick={() => toggleTimer(p.id)}>
                    <Card 
                      ...
                      // Se elimina el onClick aquí para que no interfiera con el SortableItem
                      role="button"
                      tabIndex={0}>
